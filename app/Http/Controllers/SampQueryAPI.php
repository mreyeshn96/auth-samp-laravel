...
class SampQueryAPI extends Controller
{
    /**
     *	@ignore
     */
    ...
    {
        $this->aServer[0] = env("API_SAMP_ADDRESS");
        $this->aServer[1] = env("API_SAMP_PORT");
    ...
    public function getInfo()
    {
        ...
        $aDetails['password'] = (int) ord(fread($this->rSocket, 1));

        $aDetails['players'] = (int) $this->toInteger(fread($this->rSocket, 2));

        $aDetails['maxplayers'] = (int) $this->toInteger(fread($this->rSocket, 2));
        ...
        if ($iPlayerCount > 0) {
            for ($iIndex = 0; $iIndex < $iPlayerCount; ++$iIndex) {
                ...
                $aDetails[] = array(
                    ...
                    "score" => (int) $this->toInteger(fread($this->rSocket, 4)),
                );
            }
        }
    ...
    public function getDetailedPlayers()
    {
        ...
        for ($iIndex = 0; $iIndex < $iPlayerCount; ++$iIndex) {
            $aPlayer['playerid'] = (int) ord(fread($this->rSocket, 1));

            $iStrlen = ord(fread($this->rSocket, 1));
            $aPlayer['nickname'] = (string) fread($this->rSocket, $iStrlen);

            $aPlayer['score'] = (int) $this->toInteger(fread($this->rSocket, 4));
            $aPlayer['ping'] = (int) $this->toInteger(fread($this->rSocket, 4));
